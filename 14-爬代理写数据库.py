...
# 打开数据库连接
db = pymysql.connect("66.98.117.31","root","buptisc","proxy" )
...
class proxy:
    ...
    def return_proxy(self,choice='http', first=1, end=2):
        global cursor
        user_agent = self.get_user_agent()
        headers = {'User-Agent': user_agent}
        ip_list = self.get_proxy(headers=headers, choice=choice, first=first, end=end)
        # with codecs.open("Http_Agent.txt", 'a', 'utf-8') as file:
        for ip_port in ip_list:
            ip = self.test_proxy(ip_port, choice=choice)
            if ip:
                cursor.execute("insert into proxy_now values('"+ip_port+"')")
                print("已加:",ip_port)


px = proxy()
px.return_proxy('http')
